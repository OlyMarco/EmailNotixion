...
- **⏰ 定时重建连接** - 每2分钟自动重建所有邮箱连接，彻底解决长时间运行卡死问题
- **⚡ 超时保护机制** - 所有IMAP操作设置30秒超时，避免单个账号卡死影响整体服务
...
### Q: v1.0.7版本有什么重要改进？
A: v1.0.7版本专注解决长时间运行卡死问题：
- **简化策略**：移除复杂的健康检查，采用每2分钟重建连接的直接策略
- **超时保护**：所有操作都有30秒超时，避免单个操作卡死
- **更加稳定**：代码更简洁，错误处理更直接，稳定性大幅提升

### Q: 从旧版本升级需要重新配置吗？
A: 不需要。v1.0.7版本完全向后兼容，所有现有配置（邮箱账号、推送目标、间隔设置等）都会自动保留。
