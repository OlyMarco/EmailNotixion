...
### Q: 什么是HTML邮件和富文本邮件？
A: 这是v1.0.4版本持续优化的重要功能：
- **HTML邮件**: 包含HTML格式的邮件，如系统通知、营销邮件等
- **富文本邮件**: 包含复杂格式的邮件，如表格、样式等
- 插件会自动识别并转换为易读的纯文本格式

### Q: v1.0.4版本有什么主要改进？
A: v1.0.4版本主要关注跨平台兼容性和消息发送准确性：
- **跨平台兼容性**：优化了在不同消息平台（QQ、微信、钉钉等）上的表现
- **消息发送准确性**：统一使用 event.send() 方法，消除重复发送问题
- **性能优化**：异步非阻塞设计，提升多账号处理效率
- **稳定性提升**：增强错误处理，提高长期运行稳定性
...
### v1.0.4 持续优化功能
本版本在原有HTML邮件和富文本支持基础上，重点优化了跨平台兼容性：
